            ...
            <div class="modal-body">
                ...
                <div class="mb-3">
                    ...
                    <div class="modal-footer">
                        ...
                        <button type="submit" name="add_sticker" class="btn btn-primary">Add</button>
                    </div>
                </form>
            </div>
        </div>
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form method="post">
                <div class="mb-3">
                        <input type="hidden" name="sticker_id" id="edit_id">
                        <label for="exampleInputEmail1" class="form-label">Name</label>
                    ...
                    <!-- <div class="mb-3">
                        <label for="exampleInputEmail1" class="form-label">Price</label>
                        <input type="number" class="form-control" id="edit_total" name="total">
                    ...
                    <div class="modal-footer">
                        <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Close</button>
                        <button type="submit" name="edit_sticker" class="btn btn-primary">Edit</button>
                    </div>
                </form>
            </div>
        </div>
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form action="" method="POST">
                    <input type="hidden" name="stickeriddelete" id="delete_id">
            ...
            <div class="modal-footer">
                ...
                <button type="submit" name="delete_sticker" class="btn btn-danger">Yes</button>
            </div>
            </form>
        </div>
    </div>
    ...
    <div class="row g-4">
        ...
        <div class="col-12">
            <div class="bg-light rounded h-100 p-4">
                <h4 class="mb-3">Add Sticker</h4>
                <button type="button" class="btn btn-primary my-4" data-bs-toggle="modal" data-bs-target="#addModal">Add sticker<i class="fa fa-plus ms-2"></i></button>
                <div class="table-responsive">
                    <table class="table">
                        ...
                        <tbody>
                            <?php
                            $id = 1;
                            $query = $connection->query('SELECT * FROM `sticker`');
...
if (isset($_POST['add_sticker'])) {
    ...
    $query = $connection->query("INSERT INTO `sticker`(`name`,`quantity`, `price`, `total`) VALUES ('$name','$quantity','$price','$total')");
    if ($query) {

        echo "<script>window.location='sticker.php?msg=update-sucessfully'</script>";
    }
}


if (isset($_POST['delete_sticker'])) {
    $id = $_POST['stickeriddelete'];

    $query = $connection->query('DELETE FROM `sticker` WHERE id =' . $id);
    if ($query) {
        echo "<script>window.location='sticker.php?msg=update-sucessfully'</script>";
    }
}
...
;
    ...
    $update = $connection->query("UPDATE `sticker` SET `name`='$name',`quantity`='$quantity',`price`='$price',`total`='$total' WHERE id =".$id);
    if ($update) {

        echo "<script>window.location='sticker.php?msg=update-yes'</script>";
    }
}
